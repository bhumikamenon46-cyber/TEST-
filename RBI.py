# Bhum_dashboard_enhanced_RBI.py - Modified for Dark Theme and RBI Context
...
# ---------- PAGE CONFIG ----------
st.set_page_config(page_title="RBI Monetary Policy Shock Simulation Dashboard", layout="wide")

# ---------- DARK THEME ENFORCEMENT (Custom CSS) ----------
# Enforce Dark Theme by setting default and applying custom styles
st.markdown(
    """
    <style>
    /* Force dark background for the whole app */
    .stApp { background-color: #0f1724; color: #e6eef8; }
    /* Style for buttons (optional) */
    .stButton>button { background-color:#0b84ff; color: white; }
    /* Ensure text in main content is light */
    .css-1d391kg { color: #e6eef8; }
    /* Ensure text input and select box text is readable */
    .stTextInput>div>div>input { color: #e6eef8; }
    .stSelectbox>div>div>div>div { color: #e6eef8; }
    /* Sidebar styling */
    .css-1lcbmhc, .css-1lcbmhc a { color: #e6eef8; }
    /* Change the color of the subheaders if they are not already styled by the main text color */
    h2, h3, h4 { color: #8ab4f8; } 
    </style>
    """,
    unsafe_allow_html=True,
)

# New Title for RBI Internship Context
st.title("🏦 RBI Monetary Policy Shock Simulation & Risk Assessment")
st.markdown(
    "Interactive simulation of monetary policy shocks with uncertainty, per-capita views, and integrated risk scoring for macroeconomic stability analysis."
)

# ---------- SIDEBAR INPUTS ----------
# Removed Theme Selector from original code
...
if scenario != "Custom":
    # Re-assign values based on preset to the actual variables
    ...
    # Manually update the sliders/inputs if needed, though Streamlit will re-render
    # For a clean UX, the user might expect the sliders to move when the preset is selected.
    # In a real app, you might use session state to drive the slider defaults, but here
    # we just update the variables, and the Streamlit code structure re-reads the inputs
    # on each run. The preset logic needs to override the slider values if it is not "Custom".
    # A cleaner approach for Streamlit would be:
    # 1. Read the preset selection.
    # 2. If not "Custom", use the preset values to set the slider defaults/values.
    # We will keep the current structure, which assigns the scenario values to the variables, 
    # effectively overriding the slider's current state for the simulation logic.
...
# ---------- ADVANCED / NONLINEAR SIMULATION (Remaining logic as is) ----------
...
# Apply style formatting without explicit theme styling; Streamlit handles table styles best.
...
st.markdown("""
This tool simulates the potential effects of *Monetary Policy Shocks* (interest rate, liquidity, and inflation changes) on key macroeconomic variables. It's designed for quick, scenario-based analysis.

- *Risk Score:* An aggregate indicator of scenario volatility and departure from baseline conditions.
- *Real Policy Rate:* Crucial metric calculated as *Nominal Rate - Inflation Rate*. A higher real rate generally suggests tighter monetary conditions.
- *Confidence Bands:* Illustrate the range of uncertainty (best/worst case) associated with the projections due to potential macroeconomic volatility.
- *Risk Contributions:* Helps to isolate which policy change (rate, liquidity, or inflation) is the primary driver of the overall scenario risk.

*Usage Tip:* This dashboard is an exploratory tool. The underlying model is simplified and non-linear, meaning small changes can have disproportionate effects, reflecting real-world policy complexity.
""")

# ---------- DOWNLOAD BUTTON ----------
st.subheader("Download Simulation Data")
csv = data.reset_index().to_csv(index=False).encode("utf-8")
st.download_button("Download full simulation CSV", csv, file_name="rbi_monetary_simulation.csv", mime="text/csv")
...
st.markdown(
    """
    ---
    *For RBI Internal Use:* - This model provides indicative directional assessment. For official policy recommendations, full structural macro-econometric models must be employed.
    - Internship Project: Monetary Policy Risk Simulation.
    """
)
